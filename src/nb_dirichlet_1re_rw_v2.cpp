...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_IVECTOR(pred_factor3k_h);
  DATA_IVECTOR(pred_factor3k_levels);
  // composition data
  ...
  vector<Type> pred_factor2k_i(n_pred_levels); // vector of predicted random factor levels
  ...
  // Include or exclude RE predictions
  pred_eff = X2_pred_ij * b2_jg;    
  
  if (re_predictions) {
    for (int i = 0; i < n_pred_levels; ++i) {
      for(int k = 0; k < n_cat; k++) {
        pred_eff(i, k) = pred_eff(i, k) + z2_k(pred_factor2k_i(i));
      }
    }
  } 
