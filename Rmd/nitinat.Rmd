...
The south coast stock composition and catch/effort data are currently binned at the creel survey subarea level, which approximately correspond to PFMA subareas. The relevant Nitinat creel survey areas include 121A and 21A, as well as 121C (121A corresponds to 121-1, 21A to 21-0 and 121C spans an inshore portion of 121-1 and 121-2). Here I fit a model generating predictions of a) stock-composition and b) stock-specific abundance (a function of stock composition and total abundance based on fishery catch per unit effort). This model framework has been peer reviewed (Freshwater et al. 2021).

Model predictions, rather than raw data alone, are helpful for generating predictions in this context because of substantial heterogeneity in sampling coverage (Figure \ref{fig:figs1}). In particular there are no composition data for May and very little for September in the subareas of interest. The model generates predictions in data-limited strata using data from subareas in relatively close proximity. Additionally a small number of subareas were pooled due to small sample sizes or changes in naming through time (e.g. 21A and 121C). GSI data were binned to emphasize Nitinat, southwest coast Vancouver Island, and Fraser stock groups.

```{r exp_figs, echo = FALSE, fig.cap="\\label{fig:figs1}Number of GSI samples available by biweekly sampling event, year and creel survey subarea. Colors represent subareas in immediate proximity to Nitinat mouth (red) or in nearby areas (blue)."}
...
Both raw stock composition data (Figure \ref{fig:figs2}) and model predicted stock compositions (Figure \ref{fig:figs3}) highlight strong seasonal changes in relative abundance. Large contributions of Puget Sound and non-focal stocks (primarily Columbia River) dominate 121A and 21A-121C early in the year. Fraser Summer 4.1 fish (predominantly South Thompson) are relatively abundant in 121A in July and August, while SWVI and Nitinat stocks are highly abundant in 21A-121C in August and September. These reflect three distinct marine life histories:
...
2. Fraser Summer 4.1 migrate through the area during late summer. Like Puget Sound fish, they are relatively more abundant in 121A than 21A, however their migration is more compressed in time.
3. SWVI and Nitinat populations are rare until mid-August at which point they are the dominant stocks in nearshore (e.g. 21A, 123R), but not offshore, areas. Their total abundance is substantially smaller than Puget Sound or Fraser Summer Run populations. 
