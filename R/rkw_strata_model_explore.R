...
east_dat <- comp_in %>% 
  mutate(
    ...
    strata = as.factor(strata2),
    strata_region = ifelse(
      grepl("n_haro", strata) | grepl("s_haro", strata) | grepl("vic", strata),
      "haro_vic",
      "sooke"
    ) %>% 
      factor()
  ...
  mutate(
    pred_dat = purrr::map(
      ...
      ~ expand.grid(
        ...
        left_join(
          ., 
          east_dat %>% select(strata, strata_region) %>% distinct(),
          by = "strata"
        )
    )
  )
...
# as above but full cycle
east_fit2 <- fit_stockseasonr(
  comp_formula = stock_group2 ~ 1 + strata +
...
# as above but stratified by sooke/vic vs haro
east_fit3 <- fit_stockseasonr(
  comp_formula = stock_group2 ~ 1 + strata +
    s(month_n, bs = "cc", k = 4, by = strata_region) +
    (1 | year),
  comp_dat = dat_tbl$data[[4]],
  pred_dat = dat_tbl$pred_dat[[4]],
  model = "dirichlet",
...
pred_east2 <- clean_pred_foo(fit = east_fit2, preds = dat_tbl$pred_dat[[4]])
pred_east3 <- clean_pred_foo(fit = east_fit3, preds = dat_tbl$pred_dat[[4]])
...
preds <- rbind(
  ...
  pred_east$preds %>%
    mutate(region = "east"),
  pred_east2$preds %>%
    mutate(region = "east_cc"),
  pred_east3$preds %>%
    mutate(region = "east_cc_strata")
  ) %>% 
  mutate(
    stock = factor(
      stock,
...
# compare models for western JdF
preds_west <- preds %>% 
...
p_ribbon_west <- p_west +
  geom_ribbon(data = preds_west,
              aes(ymin = pred_prob_low, ymax = pred_prob_up, fill = strata),
              alpha = 0.2) 

p_obs_west <- ggplot() +
  geom_jitter(data = obs_west,
              aes(x = month_n, y = obs_ppn, colour = strata, 
                  size = samp_nn),
              alpha = 0.1) +
  geom_line(data = preds_west,
...
pdf(here::here("figs", "rkw_habitat", "western_model_comp.pdf"),
    height =  4.5, width = 8)
p_west
p_obs_west
dev.off()
# large unrealistic deviations for WCVI composition early in year


# compare models for easter JdF and Haro
preds_east <- preds %>% 
  filter(!region %in% c("swift", "all"),
         month_n >= 4)
obs_east <- obs %>% 
  filter(!region %in% c("swift", "all"), 
         month_n >= 4)

p_east <- ggplot(data = preds_east, 
                 aes(x = month_n, colour = strata)) +
  labs(y = "Predicted Stock Proportion", x = "Month") +
  facet_grid(region~stock) +
  ggsidekick::theme_sleek() +
...
p_ribbon_east <- p_east +
  geom_ribbon(data = preds_east,
              aes(ymin = pred_prob_low, ymax = pred_prob_up, fill = strata),
              alpha = 0.2) 

p_obs_east <- ggplot() +
  geom_jitter(data = obs_east,
              aes(x = month_n, y = obs_ppn, colour = strata, 
                  size = samp_nn),
              alpha = 0.1) +
  geom_line(data = preds_east,
            aes(x = month_n, colour = strata, y = pred_prob_est),
            linewidth = 1.25) +
  labs(y = "Predicted Stock Proportion", x = "Month") +
  facet_grid(region~stock) +
...
ggplot() +
  geom_jitter(data = obs_east %>% filter(region == "east_cc_strata"),
              aes(x = month_n, y = obs_ppn, size = samp_nn, colour = strata),
              alpha = 0.1, width = 0.1) +
  geom_line(data = preds_east %>% filter(strata == "victoria"),
            aes(x = month_n, y = pred_prob_est, colour = region),
            linewidth = 1.25) +
  labs(y = "Predicted Stock Proportion", x = "Month") +
  facet_wrap(~stock) +
  ggsidekick::theme_sleek() +
